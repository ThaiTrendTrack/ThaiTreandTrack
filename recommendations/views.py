...
def search_movie_by_title(request):
    ...
    try:
        # Filter movies by title (case-insensitive match)
        filtered_movies = Movie.objects.filter(title_en__icontains=search_query)

        # Check if any movies are found
        if not filtered_movies:
            return JsonResponse({"error": "No movies found for the search query."}, status=404)

        # Prepare the response data for the filtered movies
        response_data = [
            {
                'id': movie.id,
        ...
        print(f"Error: {str(e)}")
        return JsonResponse({"error": "Something went wrong during the search."}, status=500)
